(async function () {
    ...
    const Router = {
        init() {
            ...
            // 注意：将 route() 放在最后执行，确保所有的事件监听器都已附加

            // 1. 获取所有必需的元素
            const elSidebar = document.querySelector('.sidebar');
            const elMenuToggleBtn = document.getElementById('menu-toggle-btn');
            const elApp = document.getElementById('app');

            // 2. 只有当侧边栏和菜单按钮都存在时，才添加移动端交互逻辑
            if (elSidebar && elMenuToggleBtn) {

                // 1. 菜单按钮点击事件：切换侧边栏状态
                elMenuToggleBtn.addEventListener('click', (e) => {
                    // 仅用于调试：在控制台打印消息，检查点击事件是否被触发
                    console.log('Menu toggle button clicked. Toggling sidebar.');
                    e.stopPropagation();
                    elSidebar.classList.toggle('sidebar-open');
                });

                // 2. 修复点击侧边栏链接后侧边栏不关闭的问题
                const navLinks = elSidebar.querySelectorAll('.nav-item');
                ...
                // 3. 点击内容区域关闭侧边栏 (已修复潜在的崩溃错误)
                if (elApp) {
                    elApp.addEventListener('click', () => {
                        // 仅在移动端侧边栏打开时关闭
                        if (window.innerWidth <= 768 && elSidebar.classList.contains('sidebar-open')) {
                            elSidebar.classList.remove('sidebar-open');
                        }
                    });
                }
